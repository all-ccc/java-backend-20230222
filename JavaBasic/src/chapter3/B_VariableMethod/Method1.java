...
// 작업을 미리 작성해둔 것
// 메서드에 넣을 값과 반환값만 알면 됨 (Black Box 구조)

// 메서드의 장점
// 1. 높은 재사용성 : 한 번 만들어 놓은 메서드를 반복해서 사용 가능
// 2. 중복된 코드 제거 : 반복적으로 나타나는 문장들을 메서드로 만들어서 사용하면
//					코드의 중복을 제거할 수 있고 수정이 발생하면 하나의 코드만 수정하면 됨
// 3. 프로그램 구조화 : 작업 단위로 여러 개의 메서드에 담아 프로그램을 구조화할 수 있음

// 파라미터가 있는 메서드는 반드시 파라미터 범위 등과 같은 검증을 먼저 해줘야 함

class HigherMathematics {
	final static double PIE = 3.14;	// static 변수 -> 인스턴스 메소드나 스태틱 메소드 내에서 둘 다 사용 가능
	...
	static double getTriangleSize(double underside, double height) {
		...
		return size;	// 어떤 값을 리턴해주는지 이름 잘 적어야
	}
	...
	static double getCircleSize(double radius) {
		...
		double size = PIE * radius * radius;	// PIE가 static 변수 아니면 오류남
		return size;
	}
