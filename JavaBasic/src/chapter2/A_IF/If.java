...
public class If {
	...
	public static void main(String[] args) {
		...
		// if - else문
		// 조건이 true일 경우와 false일 경우에 실행되는 코드를 각각 지정한 구문
		...
		// if문과 마찬가지로 else 문도 코드 블럭에 오는 실행할 코드가
		...
		// if - else if - else문
		// 중첩 if문의 응용
		// else문의 코드 블럭에 오는 코드가 한 줄일 때 {}을 제거 가능함을 이용
		if (age > 19) {
			System.out.println("성인입니다.");
		} else if (age < 8) {
...
//		}
		...
		if(!normal) System.out.println("비정상1"); // 비정상이 빠지도록
		boolean normal2 = true;
		if(!normal2) System.out.println("비정상2");
		System.out.println("정상 결과"); // 정상은 일자로
	}
